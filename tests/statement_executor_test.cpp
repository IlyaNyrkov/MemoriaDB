...
static Insert insertRows(std::string table,
...
static Update updateSet(std::string table,
...
TEST(StatementExecutor, ExecInsert_MultiRow) {
    ...
    exec.execInsert(insertRows("t", {},
        {{ VStr("a"), VInt(1) }, { VStr("b"), VInt(2) }}));
...
TEST(StatementExecutor, ExecDelete_WithWhere) {
    ...
    size_t removed = exec.execDelete(deleteFrom("t",
        W(Cmp("c2", CompareOp::Ge, VInt(2)))
    ));
...
TEST(StatementExecutor, ExecUpdate_SetWhere) {
    ...
    size_t changed = exec.execUpdate(updateSet("t", sets,
        W(Cmp("c1", CompareOp::Eq, VStr("a")))
    ));
...
TEST(StatementExecutor, ExecSelect_ProjectionAndWhere) {
    ...
    Select sel = selectCols("t", {"c1"},
        W(Cmp("c2", CompareOp::Gt, VInt(1)))
    );
...
TEST(StatementExecutor, ExecSelect_WhereTypeMismatchThrows) {
    ...
    try {
        (void)exec.execSelect(selectStar("t",
            W(Cmp("c1", CompareOp::Ge, VInt(2)))
        ));
