...
TEST(Table, SelectWhere_FullRows) {
  ...
  auto rows = t.getRowsWhere(
      [](const Row &r) { return std::get<std::string>(r.at(0)) == "b"; });
...
TEST(Table, GetRowsWhere_InvalidProjectionIndexThrows) {
  ...
  try {
    // Use your current method name; if it's getRowsWhere(), replace below
    // accordingly.
...
TEST(Table, UpdateWhere_Assignments) {
  ...
  size_t updated =
      t.updateWhere([](const Row &r) { return asInt(r, 1) < 3; }, assigns);
  EXPECT_EQ(updated, 2u);

  auto rows = t.getRowsWhere([](const Row &) { return true; });
  ASSERT_EQ(rows.size(), 3u);
  EXPECT_EQ(asStr(rows.at(0), 0), "new");
  EXPECT_EQ(asInt(rows.at(0), 1), 1);
  EXPECT_EQ(asStr(rows.at(1), 0), "new");
  EXPECT_EQ(asInt(rows.at(1), 1), 2);
  EXPECT_EQ(asStr(rows.at(2), 0), "y");
  EXPECT_EQ(asInt(rows.at(2), 1), 3);
}
