...
static bool whereEq(const WhereExpr& A, const WhereExpr& B) {
    return std::visit([&](auto const& x)->bool {
        ...
        if constexpr (std::is_same_v<T, Comparison>) {
            if (!std::holds_alternative<Comparison>(B)) return false;
...
TEST(Parser, Update_Set_WithWhere_AND) {
    ...
    WhereExpr expWhere = WAnd(
        W(Cmp("c2", CompareOp::Ge, RowValue{int64_t{3}})),
...
TEST(Parser, Select_Star_WithWhere_OR) {
    ...
    WhereExpr expWhere = WOr(
        W(Cmp("c2", CompareOp::Lt, RowValue{int64_t{5}})),
