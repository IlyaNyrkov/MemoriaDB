...
std::optional<QueryResult> StatementExecutor::execute(const Statement& st) {
    return std::visit([&](const auto& node) -> std::optional<QueryResult> {
...
StatementExecutor::Pred
StatementExecutor::compileWhere(const WhereExpr& expr, const Schema& schema) const {
    // comparison node
    auto cmp = [&](const Comparison& c) -> Pred {
        ...
        if (t == ColumnType::Int) {
            ...
            switch (c.op) {
                case CompareOp::Eq:  return [=](const Row& r){ return std::get<int64_t>(r.at(idx)) == rhs; };
                case CompareOp::Neq: return [=](const Row& r){ return std::get<int64_t>(r.at(idx)) != rhs; };
                case CompareOp::Lt:  return [=](const Row& r){ return std::get<int64_t>(r.at(idx)) <  rhs; };
                case CompareOp::Gt:  return [=](const Row& r){ return std::get<int64_t>(r.at(idx)) >  rhs; };
                case CompareOp::Le:  return [=](const Row& r){ return std::get<int64_t>(r.at(idx)) <= rhs; };
                case CompareOp::Ge:  return [=](const Row& r){ return std::get<int64_t>(r.at(idx)) >= rhs; };
            }
        } else { // string column
            ...
            switch (c.op) {
                case CompareOp::Eq:  return [=](const Row& r){ return std::get<std::string>(r.at(idx)) == rhs; };
                case CompareOp::Neq: return [=](const Row& r){ return std::get<std::string>(r.at(idx)) != rhs; };
                default:
    ...
    // recursion
    return std::visit([&](const auto& node) -> Pred {
        using T = std::decay_t<decltype(node)>;
...
StatementExecutor::compileProjection(const Select::Projection& proj, const Schema& schema) const {
    if (std::holds_alternative<Select::Star>(proj)) {
        std::vector<std::size_t> idx(schema.size());
        for (std::size_t i = 0; i < schema.size(); ++i) idx[i] = i;
        return idx;
    }
...
std::vector<std::size_t>
StatementExecutor::compileInsertColumnOrder(const Insert& st, const Schema& schema) const {
    std::vector<std::size_t> order;

    if (st.columnNames.empty()) {
        order.resize(schema.size());
        for (std::size_t i = 0; i < schema.size(); ++i) order[i] = i;
        return order;
    }
