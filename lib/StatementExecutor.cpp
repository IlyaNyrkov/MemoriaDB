...
QueryResult StatementExecutor::execSelect(const Select &st) const {
  ...
  if (std::holds_alternative<Select::Star>(st.projection)) {
    ...
    out.header = names;
    out.rows =
        tbl.getColumnRowsWhere(indices, pred); // <-- assign, no push_back
  }
...
StatementExecutor::compileWhere(const WhereExpr &expr,
    ...
    if (t == ColumnType::Int) {
      if (!std::holds_alternative<int64_t>(c.literal))
        throw std::invalid_argument(
            "WHERE type mismatch: expected int literal");

      const int64_t rhs = std::get<int64_t>(c.literal);
      switch (c.op) {
      case CompareOp::Eq:
        return
            [=](const Row &r) { return std::get<int64_t>(r.at(idx)) == rhs; };
      case CompareOp::Neq:
        return
            [=](const Row &r) { return std::get<int64_t>(r.at(idx)) != rhs; };
      case CompareOp::Lt:
      ...
      if (!std::holds_alternative<std::string>(c.literal))
        throw std::invalid_argument(
            "WHERE type mismatch: expected string literal");

      const std::string rhs = std::get<std::string>(c.literal);
      switch (c.op) {
      case CompareOp::Eq:
        return [=](const Row &r) {
          return std::get<std::string>(r.at(idx)) == rhs;
        };
      case CompareOp::Neq:
        return [=](const Row &r) {
          return std::get<std::string>(r.at(idx)) != rhs;
  ...
  return std::visit(
      [&](const auto &node) -> Pred {
        ...
        if constexpr (std::is_same_v<T, Comparison>) {
          ...
          return [L = std::move(L), R = std::move(R)](const Row &r) {
            return L(r) && R(r);
          };
...
StatementExecutor::compileProjection(const Select::Projection &proj,
                                     const Schema &schema) const {
...
StatementExecutor::compileAssignments(const std::vector<Assignment> &sets,
  ...
  for (const auto &a : sets) {
    ...
    if (!valueTypeMatches(t, a.value))
      throw std::invalid_argument("UPDATE type mismatch for column '" +
                                  a.column + "'");
...
std::vector<std::size_t>
StatementExecutor::compileInsertColumnOrder(const Insert &st,
                                            const Schema &schema) const {
...
Row StatementExecutor::makeRowForInsert(
    const std::vector<RowValue> &values,
    const std::vector<std::size_t> &columnOrder, const Schema &schema) const {
