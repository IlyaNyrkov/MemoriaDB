...
namespace memoria {
    ...
    static std::string parseIdent(std::string_view s, std::size_t& i) {
        ...
        while (j < s.size()) {
            char c = s[j];
            if (std::isalnum(static_cast<unsigned char>(c)) || c == '_') { ++j; continue; }
            break;
        }
    ...
    static int64_t parseInt64(std::string_view s, std::size_t& i) {
        ...
        if (res.ec != std::errc{}) throw ParseError("Invalid integer literal");
        return v;
    }

    static std::string parseQuoted(std::string_view s, std::size_t& i) {
        skipSpaces(s, i);
        if (i >= s.size() || (s[i] != '\'' && s[i] != '"')) throw ParseError("Expected quoted string");
        char quote = s[i++];
        std::string out;
        while (i < s.size()) {
            char c = s[i++];
            if (c == quote) {
                // simple SQL-style doubling not supported; assignment spec keeps it simple
                return out;
            }
            out.push_back(c);
        }
...
Parser::peelWhere(std::string_view normalized) {
    ...
    for (std::size_t i = 0; i < normalized.size(); ++i) {
        ...
        if ((c == '\'' || c == '"')) {
            ...
            if (i + KW.size() <= normalized.size() &&
                normalized.substr(i, KW.size()) == KW) {
                // Ensure there's whitespace before 'W' or it's not needed (start)
                if (i > 0 && !is_space(normalized[i - 1])) continue;
                std::size_t tail = i + KW.size();
                // consume following spaces
                while (tail < normalized.size() && is_space(normalized[tail])) ++tail;
                ...
                while (bj > 0 && is_space(base[bj - 1])) --bj;
                base.resize(bj);
                return {std::move(base), where.empty() ? std::optional<std::string>{} : std::optional<std::string>{std::move(where)}};
            }
        }
    }
    // no WHERE
...
static CompareOp parseOp(std::string_view s, std::size_t& i) {
    skipSpaces(s, i);
    if (i + 2 <= s.size()) {
        if (s[i] == '!' && s[i+1] == '=') { i += 2; return CompareOp::Neq; }
        if (s[i] == '<' && s[i+1] == '=') { i += 2; return CompareOp::Le; }
        if (s[i] == '>' && s[i+1] == '=') { i += 2; return CompareOp::Ge; }
    }
    if (i < s.size()) {
        if (s[i] == '=') { ++i; return CompareOp::Eq; }
        if (s[i] == '<') { ++i; return CompareOp::Lt; }
        if (s[i] == '>') { ++i; return CompareOp::Gt; }
    }
...
static RowValue parseLiteral(std::string_view s, std::size_t& i) {
    skipSpaces(s, i);
    if (i >= s.size()) throw ParseError("Expected literal");
    if (s[i] == '\'' || s[i] == '"') {
        ...
        auto parseOr = [&](auto&& self)->WhereExpr {
            ...
            auto parseAnd = [&](auto&& self2)->WhereExpr {
                ...
                while (true) {
                    ...
                    if (i + 3 <= s.size() && s.substr(i, 3) == "AND") {
                        // ensure delimiter
                        if (i+3 < s.size() && std::isalnum(static_cast<unsigned char>(s[i+3]))) { i = save; break; }
                        i += 3;
            ...
            while (true) {
                ...
                if (i + 2 <= s.size() && s.substr(i, 2) == "OR") {
                    if (i+2 < s.size() && std::isalnum(static_cast<unsigned char>(s[i+2]))) { i = save; break; }
                    i += 2;
...
WhereExpr Parser::parseWhere(std::string_view whereTail) {
    ...
    auto parseOr = [&](auto&& self)->WhereExpr {
        ...
        auto parseAnd = [&](auto&& self2)->WhereExpr {
            ...
            while (true) {
                ...
                if (i + 3 <= whereTail.size() && whereTail.substr(i, 3) == "AND") {
                    if (i+3 < whereTail.size() && std::isalnum(static_cast<unsigned char>(whereTail[i+3]))) { i = save; break; }
                    i += 3;
        ...
        while (true) {
            ...
            if (i + 2 <= whereTail.size() && whereTail.substr(i, 2) == "OR") {
                if (i+2 < whereTail.size() && std::isalnum(static_cast<unsigned char>(whereTail[i+2]))) { i = save; break; }
                i += 2;
                WhereExpr right = parseAnd(parseAnd);
                Or o;
                o.lhs = std::make_unique<WhereExpr>(std::move(left));
    ...
    skipSpaces(whereTail, i);
    if (i != whereTail.size()) throw ParseError("Trailing tokens after WHERE");
    return root;
}
...
// -------------------- Base (non-WHERE) parsing --------------------
    Statement parseCreateTableStmt(std::string_view s, std::size_t& i) {
        ...
        if (i >= s.size() || s[i] != '(') throw ParseError("Expected '(' after CREATE TABLE name");
        ++i;
        ...
        skipSpaces(s, i);
        if (i != s.size()) throw ParseError("Trailing tokens after CREATE TABLE");

        // FIX: CreateTable carries a Schema
    ...
    static Statement parseInsertStmt(std::string_view s, std::size_t& i) {
        ...
        while (true) {
            ...
            memoria::skipSpaces(s, i);
            if (i < s.size() && s[i] == ',') { ++i; continue; } // next row
            break; // no more rows
        }
    ...
    static Statement parseDeleteStmt(std::string_view s, std::size_t& i) {
        ...
        skipSpaces(s, i);
        if (i != s.size()) throw ParseError("Trailing tokens after DELETE FROM");
    ...
    static Statement parseUpdateStmt(std::string_view s, std::size_t& i) {
        ...
        std::string table = parseIdent(s, i);
        skipSpaces(s, i);
        if (!starts_with(s.substr(i), "SET")) throw ParseError("Expected SET");
        ...
        while (true) {
            std::string cname = parseIdent(s, i);
            skipSpaces(s, i);
            if (i >= s.size() || s[i] != '=') throw ParseError("Expected '=' in assignment");
            ++i;
        ...
        skipSpaces(s, i);
        if (i != s.size()) throw ParseError("Trailing tokens after UPDATE");
    ...
    static Statement parseSelectStmt(std::string_view s, std::size_t& i) {
        ...
        // FROM <table>
        skipSpaces(s, i);
        if (!starts_with(s.substr(i), "FROM ")) throw ParseError("Expected FROM");
        i += std::string_view("FROM ").size();
        std::string table = parseIdent(s, i);

        // no trailing tokens in the base
        skipSpaces(s, i);
        if (i != s.size()) throw ParseError("Trailing tokens after SELECT");

        // NOTE: WHERE is handled upstream (peelWhere/parseWhere), so set where = nullopt here.
    ...
    Statement Parser::parseBase(std::string_view base) {
        ...
        if (starts_with(base.substr(i), "CREATE TABLE ")) return parseCreateTableStmt(base, i);
        if (starts_with(base.substr(i), "INSERT INTO "))  return parseInsertStmt(base, i);
        if (starts_with(base.substr(i), "DELETE FROM "))  return parseDeleteStmt(base, i);
        if (starts_with(base.substr(i), "UPDATE "))       return parseUpdateStmt(base, i);
        if (starts_with(base.substr(i), "SELECT "))       return parseSelectStmt(base, i);
