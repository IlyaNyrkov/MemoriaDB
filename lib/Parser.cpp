    ...
    if ((c == '\'' || c == '"')) {
      ...
      if (i + KW.size() <= normalized.size() &&
        ...
        return {std::move(base),
                where.empty() ? std::optional<std::string>{}
    ...
    auto parseOr = [&](auto &&self) -> WhereExpr {
      ...
      while (true) {
        ...
        if (i + 2 <= s.size() && s.substr(i, 2) == "OR") {
          if (i + 2 < s.size() &&
              std::isalnum(static_cast<unsigned char>(s[i + 2]))) {
            i = save;
            break;
          }
          i += 2;
...
static Statement parseInsertStmt(std::string_view s, std::size_t &i) {
  ...
  return Statement{
      Insert{std::move(table), std::move(columnNames), std::move(rows)}};
}
...
static Statement parseSelectStmt(std::string_view s, std::size_t &i) {
  ...
  Select::Projection
      proj; // alias: std::variant<Select::Star, std::vector<std::string>>
  skipSpaces(s, i);
  if (i < s.size() && s[i] == '*') {
    ++i;
    proj = Select::Star{};
