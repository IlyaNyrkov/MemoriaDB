    ...
    public:
        ...
        std::size_t updateWhere(Pred pred,
            ...
            for (const auto& [idx, val] : assignments) {
                ...
                const bool ok = (col.type == ColumnType::Int && std::holds_alternative<int64_t>(val)) ||
                                (col.type == ColumnType::Str && std::holds_alternative<std::string>(val));
                if (!ok) throw std::invalid_argument("Assignment type mismatch");
            }
        ...
        template <class Pred>
        [[nodiscard]] std::vector<Row> getRowsWhere(Pred pred) const {
            std::vector<Row> out;
        ...
        [[nodiscard]] std::vector<Row> getColumnRowsWhere(const std::vector<std::size_t>& columnIndices,
            ...
            for (auto idx : columnIndices) {
                if (idx >= schema_.size()) throw std::out_of_range("Projection index out of range");
            }
